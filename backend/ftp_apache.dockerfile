# Building a docker file that supports apache and ftp
...
# Stage 2 get the apache and the built ftpd in the last stage
FROM php:8.0-apache

# install dependencies for ftpd
...
# setup run/init file
COPY ftp_run.sh /ftp_run.sh
COPY upload_start.sh /upload_start.sh
RUN chmod u+x /ftp_run.sh
RUN chmod u+x /upload_start.sh
